...
# meta developer: @kamekuro=хуесос, баннеры, фикшенный унисон, фикшенное получение треков - @codrago
# scope: heroku_only
...
class Banners:
    ...
    def ultra(self):
        ...
        draw_glow.rounded_rectangle((g_x, g_y, g_x + glow_rect_size, g_y + glow_rect_size), radius=50, fill=dominant_color)
        ...
        if len(title_text) > 30: title_text = title_text[:30] + "..."
        ...
        if len(artist_text) > 45: artist_text = artist_text[:45] + "..."
        ...
        if progress_px > bar_width: progress_px = bar_width
        ...
        if len(album_text) > 50: album_text = album_text[:50] + "..."
        ...
        bbox = draw.textbbox((center_x, current_y), self.meta_info, font=font_meta, anchor="ms")
        ...
        if self.repeat_mode != "NONE":
            ...
            draw.arc([rep_x - rep_size, icon_y_center - rep_size, rep_x + rep_size, icon_y_center + rep_size], 
                     start=40, end=320, fill=(220, 220, 220, 255), width=3)
           
            draw.polygon([(rep_x + rep_size - 2, icon_y_center - 8), (rep_x + rep_size + 8, icon_y_center), (rep_x + rep_size - 8, icon_y_center + 4)], fill=(220, 220, 220, 255))
            
            if self.repeat_mode == "ONE":
                font_one = get_font(20)
                draw.text((rep_x + rep_size + 12, icon_y_center), "1", font=font_one, fill="white", anchor="lm")
        ...
        c1_box = (heart_x - c_r*2, icon_y_center - c_r - 2, heart_x, icon_y_center + c_r - 2)
        c2_box = (heart_x, icon_y_center - c_r - 2, heart_x + c_r*2, icon_y_center + c_r - 2)
        tri_points = [
        ...
        if self.is_liked:
            ...
            draw.line([(heart_x - c_r*2 + 1, icon_y_center), (heart_x, icon_y_center + heart_size + 5)], fill="red", width=3)
            draw.line([(heart_x + c_r*2 - 1, icon_y_center), (heart_x, icon_y_center + heart_size + 5)], fill="red", width=3)
...
class YaMusicMod(loader.Module):
    ...
    async def ynowcmd(self, message: telethon.types.Message):
        ...
        try:
             # Получаем трек заново, чтобы обновить данные о лайках
        ...
        genre_map = {
            'rusrap': 'Русский рэп', 'pop': 'Поп', 'rock': 'Рок', 
            'alternative': 'Альтернатива', 'electronics': 'Электроника', 
            'hip-hop': 'Хип-хоп', 'rap': 'Рэп', 'rnb': 'R&B', 'metal': 'Метал',
            'indie': 'Инди', 'folk': 'Фолк', 'soundtrack': 'Саундтрек'
        }
        ...
        banners = Banners(
            ...
            track_cover=requests.get(f"https://{track_object.cover_uri[:-2]}1000x1000").content,
            album_title=album_title,
            meta_info=meta_info,
            is_liked=is_liked,
            repeat_mode=repeat_mode
        )
        file = getattr(banners, self.config["banner_version"], banners.ultra)()
    ...
    async def ynowtcmd(self, message: telethon.types.Message):
        ...
        await utils.answer(
            ...
            response=out,
            file=(
                await self.__download_track(ym_client, now['track']['track_id'])
            ),
            attributes=(
                [
                    telethon.types.DocumentAttributeAudio(
    ...
    async def __get_now_playing(self):
        ...
        if not ynison or (len(ynison.get("player_state", {}).get("player_queue", {}).get("playable_list", [])) == 0):
            return {}
        ...
        try:
            ...
            repeat_mode = player_state.get("player_queue", {}).get("options", {}).get("repeat_mode", "NONE")
